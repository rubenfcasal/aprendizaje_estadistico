...
```{r arbol, fig.cap="Ejemplo de un árbol de decisión obtenido al realizar una partición binaria recursiva de un espacio bidimensional.", fig.dim = c(7, 4), echo=FALSE}
...
Al final de este proceso iterativo, el espacio predictor se ha particionado en regiones de forma rectangular en las que la predicción de la respuesta es constante  `r cite_fig(predictor)`. 
...
```{r predictor, echo=FALSE, out.width="55%", fig.cap="Ejemplo de la superficie de predicción correspondiente a un árbol de decisión."}
...
La metodología CART está implementada en el paquete `r cite_cran(rpart)`  (acrónimo de *Recursive PARTitioning*)^[El paquete `r cite_cran(tree)` es una traducción del original en S.], implementado por @R-rpart. 

La función principal es `r cite_fun(rpart, rpart)` y habitualmente se emplea de la forma:
...
Emplearemos el conjunto de datos [`winequality`](https://rubenfcasal.github.io/mpae/reference/winetaste.html) del paquete `r cite_cran(mpae)`, que contiene información fisico-química 
...
Para representarlo se pueden emplear las herramientas del paquete `r cite_cran("rpart")`  `r cite_fig(arbolrpart)`:

(ref:arbolrpart) Árbol de regresión para predecir `winequality$quality` (obtenido con las opciones por defecto de `r cite_fun(rpart, rpart)`).

```{r arbolrpart, small.mar=FALSE, fig.cap='(ref:arbolrpart)'}
plot(tree)
text(tree)
```

Pero puede ser preferible emplear el paquete `r cite_cran("rpart.plot")` [@R-rpartplot]  `r cite_fig(arbolrpartplot)`:

(ref:arbolrpartplot) Representación del árbol de regresión generada con `r cite_fun(rpart.plot, rpart.plot)`.

```{r arbolrpartplot, fig.cap='(ref:arbolrpartplot)'}
library(rpart.plot)
rpart.plot(tree)  
```
...
Posteriormente, podemos emplear la función `r cite_fun(printcp, rpart)` para obtener los valores de CP para los árboles (óptimos) de menor tamaño, junto con su error de validación cruzada `xerror` (reescalado de forma que el máximo de `rel error` es 1):

```{r}
printcp(tree)
```

También `r cite_fun(plotcp, rpart)` para representarlos^[Realmente en la tabla de texto se muestra el valor mínimo de CP, ya que se obtendría la misma solución para un rango de valores de CP (desde ese valor hasta el anterior, sin incluirlo), mientras que en el gráfico generado por `r cite_fun(plotcp, rpart)` se representa la media geométrica de los extremos de ese intervalo.] `r cite_fig(cp)`: 
...
Para obtener el modelo final `r cite_fig(arbolpoda)` podamos el árbol con el valor de complejidad obtenido `r cp` (que en este caso coincide con el valor óptimo).
...
Representamos los valores observados frente a las predicciones `r cite_fig(obsXpred)`:
...
```{r obsXpred, fig.cap='(ref:obsXpred)'}
obs <- test$quality
...
Representamos los errores (reescalados) de validación cruzada `r cite_fig(errorclassif)`:

(ref:errorclassif) Evolución del error (reescalado) de validación cruzada en función del parámetro de complejidad.

<!-- printcp(tree) -->

```{r errorclassif, fig.cap='(ref:errorclassif)'}
plotcp(tree)
```

Para obtener el modelo final, seleccionamos el valor óptimo de complejidad siguiendo el criterio de un error estándar de @breiman1984classification y podamos el árbol `r cite_fig(arbolclassifpoda)`.

(ref:arbolclassifpoda) Árbol de clasificación de `winetaste$taste` obtenido después de la poda (modelo final).

```{r arbolclassifpoda, out.width=fowidth(5), fig.cap='(ref:arbolclassifpoda)'}
xerror <- tree$cptable[,"xerror"]
imin.xerror <- which.min(xerror)
upper.xerror <- xerror[imin.xerror] + tree$cptable[imin.xerror, "xstd"]
icp <- min(which(xerror <= upper.xerror))
...
El método `r cite_fun(predict.rpart, rpart)` devuelve por defecto (`type = "prob"`) una matriz con las probabilidades de cada clase, por lo que habría que establecer `type = "class"` para obtener la clase predicha (consultar la ayuda de esta función para más detalles).
...
::: {.exercise #bfan-rpart}
En este ejercicio se empleará el conjunto de datos [`mpae::bfan`](https://rubenfcasal.github.io/mpae/reference/bfan.html) del paquete `r cite_cran(mpae)` utilizado anteriormente en las secciones \@ref(reg-glm) y \@ref(generadores).
...
Si queremos emplear validación cruzada, como se hizo en el caso anterior, podemos emplear la función auxiliar `r cite_fun(trainControl, caret)`, y para considerar un mayor rango de posibles valores podemos hacer uso del argumento `tuneLength` `r cite_fig(arbolclassifggplot)`.

```{r arbolclassifggplot, out.width=fowidth(5), fig.cap="Evolución de la precisión (obtenida mediante validación cruzada) dependiendo del parámetro de complejidad, resaltando el valor óptimo."}
...
```
El modelo final se devuelve en la componente `$finalModel` `r cite_fig(arbolfinalcaret)`:

(ref:arbolfinalcaret) Árbol de clasificación de `winetaste$taste`, obtenido con la complejidad "óptima" (empleando `caret`).

```{r arbolfinalcaret, out.width=fowidth(5), fig.cap='(ref:arbolfinalcaret)'}
...
Como cabría esperar, el modelo resultante es más simple `r cite_fig(figarbolclassifoneSE)`:

```{r figarbolclassifoneSE, fig.cap='(ref:arbolclassifoneSE)'}
rpart.plot(caret.rpart$finalModel)
```

Adicionalmente, representamos la importancia de los predictores `r cite_fig(arbolImpor)`:
...
Finalmente, calculamos las predicciones con el método `r cite_fun(predict.train, caret)` y posteriormente evaluamos su precisión con `r cite_fun(confusionMatrix, caret)`:
...
También podríamos calcular las estimaciones de las probabilidades (añadiendo el argumento `type = "prob"`) y, por ejemplo, generar la curva ROC con `pROC::roc()` `r cite_fig(ROC-tree)`: 

(ref:ROC-tree) Curva ROC correspondiente al árbol de clasificación de `winetaste$taste`.

```{r ROC-tree, fig.cap="(ref:ROC-tree)"}
library(pROC)
...
* `r cite_pkg_("mlr3", "https://mlr-org.com")`, que incorpora una llamada a `rpart::rpart()` desde sus **learners** `lrn("regr.rpart")` y `lrn("classif.rpart")`.
* `r cite_pkg_("h2o", "https://www.h2o.ai/blog/finally-you-can-plot-h2o-decision-trees-in-r/")`, que aunque no ofrece una implementación de los árboles CART, sí ofrece dos alternativas más sofisticadas usando bosques aleatorios `h2o.randomForest()` y los procedimientos basados en el aumento del gradiente `h2o.gbm()`.
-->
...
Esta metodología está implementada en el paquete `r cite_pkg_("C50", "https://topepo.github.io/C5.0/index.html")` [@R-C50]. 
...
El paquete `r cite_pkg_("Cubist", "https://topepo.github.io/Cubist")` [@R-Cubist] es una evolución de M5 que incorpora un método ensemble similar a boosting.
...
Una de las más utilizadas son los *conditional inference trees* [@hothorn2006unbiased]^[Otra alternativa es GUIDE [*Generalized, Unbiased, Interaction Detection and Estimation*, @loh2002regression].], implementada en la función `r cite_fun(ctree, party)` del paquete `r cite_cran("party")` [@R-party].
...
```{r ctree-plot, fig.cap='(ref:ctree-plot)', fig.dim=c(12, 8)}
library(party)

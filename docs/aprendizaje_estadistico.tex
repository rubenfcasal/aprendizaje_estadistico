...
\hypertarget{las-dos-culturas-breiman-2001}{%
\subsection{Las dos culturas (Breiman, 2001)}\label{las-dos-culturas-breiman-2001}}
...
\NormalTok{fit}\FloatTok{.1}\NormalTok{se <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(medv }\OperatorTok{~}\StringTok{ }\KeywordTok{poly}\NormalTok{(lstat, grado}\FloatTok{.1}\NormalTok{se), train)}
...
\KeywordTok{lines}\NormalTok{(newdata}\OperatorTok{$}\NormalTok{lstat, }\KeywordTok{predict}\NormalTok{(fit}\FloatTok{.1}\NormalTok{se, }\DataTypeTok{newdata =}\NormalTok{ newdata), }\DataTypeTok{lty =} \DecValTok{2}\NormalTok{)}
\KeywordTok{legend}\NormalTok{(}\StringTok{"topright"}\NormalTok{, }\DataTypeTok{legend =} \KeywordTok{c}\NormalTok{(}\KeywordTok{paste}\NormalTok{(}\StringTok{"Grado óptimo:"}\NormalTok{, grado.op), }\KeywordTok{paste}\NormalTok{(}\StringTok{"oneSE rule:"}\NormalTok{, grado}\FloatTok{.1}\NormalTok{se)), }
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{accuracy}\NormalTok{(}\KeywordTok{predict}\NormalTok{(fit}\FloatTok{.1}\NormalTok{se, }\DataTypeTok{newdata =}\NormalTok{ test), obs)}
\end{Highlighting}
\end{Shaded}
...
Como ejemplo ilustrativo consideraremos el conocido conjunto de datos \texttt{iris} (Fisher, 1936) en el que el objetivo es clasificar flores de lirio en tres especies (\texttt{Species}) a partir del largo y ancho de sépalos y pétalos, aunque en este caso emplearemos un clasificador aleatorio.
...
Sin embargo, al aumentar el número de dimensiones se pueden agravar notablemente muchos de los problemas que ya pueden aparecer en dimensiones menores, esto es lo que se conoce como la \emph{maldición de la dimensionalidad} (\emph{curse of dimensionality}, Bellman, 1961).
...
La metodología CART (Classification and Regresion Trees, Breiman \emph{et al.}, 1984) es la más popular para la construcción de árboles de decisión y es la que se va a explicar con algo de detalle en las siguientes secciones.
...
es decir, la complejidad del modelo, seleccionando el subárbol \emph{optimo} (para los datos
...
\item
  Entropía\footnote{La entropía es un concepto básico de la teoría de la información (Shannon, 1948) y se mide en \emph{bits} (cuando en la definición se utilizan \(log_2\)).} (\emph{cross-entropy}):
...
contienen un modelo (de regresión) lineal. El paquete \href{https://topepo.github.io/Cubist}{\texttt{Cubist}} es una evolución de M5 que incorpora un método \emph{ensemble} similar a \emph{boosting}.
...
mal los valores que \emph{realmente} son muy extremos, ya que el número de posibles valores
...
predictora seleccionada, y se repite el proceso mientras haya variables \emph{significativas}.
...
Uno de los primeros métodos combinados que se utilizó fue el \emph{bagging} (nombre que viene de \emph{bootstrap aggregation}), propuesto en Breiman (1996).
...
por ejemplo se puede construir una matriz de proximidad entre observaciones a partir de la proporción de veces que están en un mismo nodo terminal (para más detalles ver \href{https://www.r-project.org/doc/Rnews/Rnews_2002-3.pdf}{Liaw y Wiener, 2002}).
...
El siguiente hito fue la aparición del método \emph{gradient boosting machine} (\href{https://projecteuclid.org/euclid.aos/1013203451}{Friedman, 2001}), perteneciente a la familia de los métodos iterativos de descenso de gradientes.
...
El propio Friedman propuso una mejora de su algoritmo (\href{https://www.sciencedirect.com/science/article/pii/S0167947301000652}{Friedman, 2002}), inspirado por la técnica bagging de Breiman. Esta variante, conocida como \emph{stochastic gradient boosting} (SGB), es a día de hoy una de las más utilizadas.
...
\CommentTok{# winetaste$taste <- factor(winetaste$taste, sort(levels(winetaste$taste)))}
\NormalTok{winetaste}\OperatorTok{$}\NormalTok{taste <-}\StringTok{ }\KeywordTok{factor}\NormalTok{(}\KeywordTok{as.character}\NormalTok{(winetaste}\OperatorTok{$}\NormalTok{taste))}
\CommentTok{# Partición de los datos}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{df <-}\StringTok{ }\NormalTok{winetaste}
\NormalTok{nobs <-}\StringTok{ }\KeywordTok{nrow}\NormalTok{(df)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pred <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(ada.boost, }\DataTypeTok{newdata =}\NormalTok{ test)}
\NormalTok{caret}\OperatorTok{::}\KeywordTok{confusionMatrix}\NormalTok{(pred, test}\OperatorTok{$}\NormalTok{taste, }\DataTypeTok{positive =} \StringTok{"good"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{confusionMatrix}\NormalTok{(}\KeywordTok{predict}\NormalTok{(caret.ada0, }\DataTypeTok{newdata =}\NormalTok{ test), test}\OperatorTok{$}\NormalTok{taste, }\DataTypeTok{positive =} \StringTok{"good"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{confusionMatrix}\NormalTok{(}\KeywordTok{predict}\NormalTok{(caret.ada1, }\DataTypeTok{newdata =}\NormalTok{ test), test}\OperatorTok{$}\NormalTok{taste, }\DataTypeTok{positive =} \StringTok{"good"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pred <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(gbm.fit, }\DataTypeTok{newdata =}\NormalTok{ test)}
\NormalTok{obs <-}\StringTok{ }\NormalTok{test}\OperatorTok{$}\NormalTok{quality}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{postResample}\NormalTok{(}\KeywordTok{predict}\NormalTok{(caret.gbm1, }\DataTypeTok{newdata =}\NormalTok{ test), test}\OperatorTok{$}\NormalTok{quality)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{caret.xgb}\OperatorTok{$}\NormalTok{bestTune}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{confusionMatrix}\NormalTok{(}\KeywordTok{predict}\NormalTok{(caret.xgb, }\DataTypeTok{newdata =}\NormalTok{ test), test}\OperatorTok{$}\NormalTok{taste)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pred <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(svm, }\DataTypeTok{newdata =}\NormalTok{ test)}
\NormalTok{caret}\OperatorTok{::}\KeywordTok{confusionMatrix}\NormalTok{(pred, test}\OperatorTok{$}\NormalTok{taste)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tuneGrid <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{sigma =} \KeywordTok{kernelf}\NormalTok{(svm)}\OperatorTok{@}\NormalTok{kpar}\OperatorTok{$}\NormalTok{sigma, }\CommentTok{# Emplea clases S4}
                       \DataTypeTok{C =} \KeywordTok{c}\NormalTok{(}\FloatTok{0.5}\NormalTok{, }\DecValTok{1}\NormalTok{, }\DecValTok{5}\NormalTok{))}
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{caret.svm <-}\StringTok{ }\KeywordTok{train}\NormalTok{(taste }\OperatorTok{~}\StringTok{ }\NormalTok{., }\DataTypeTok{data =}\NormalTok{ train,}
    \DataTypeTok{method =} \StringTok{"svmRadial"}\NormalTok{, }\DataTypeTok{preProcess =} \KeywordTok{c}\NormalTok{(}\StringTok{"center"}\NormalTok{, }\StringTok{"scale"}\NormalTok{),}
    \DataTypeTok{trControl =} \KeywordTok{trainControl}\NormalTok{(}\DataTypeTok{method =} \StringTok{"cv"}\NormalTok{, }\DataTypeTok{number =} \DecValTok{5}\NormalTok{),}
    \DataTypeTok{tuneGrid =}\NormalTok{ tuneGrid, }\DataTypeTok{prob.model =} \OtherTok{TRUE}\NormalTok{)}
\NormalTok{caret.svm}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{confusionMatrix}\NormalTok{(}\KeywordTok{predict}\NormalTok{(caret.svm, }\DataTypeTok{newdata =}\NormalTok{ test), test}\OperatorTok{$}\NormalTok{taste)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{ld.pred <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(ld, }\DataTypeTok{newdata =}\NormalTok{ test)}
\NormalTok{pred <-}\StringTok{ }\NormalTok{ld.pred}\OperatorTok{$}\NormalTok{class}
\NormalTok{caret}\OperatorTok{::}\KeywordTok{confusionMatrix}\NormalTok{(pred, test}\OperatorTok{$}\NormalTok{taste)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p.est <-ld.pred}\OperatorTok{$}\NormalTok{posterior}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{qd.pred <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(qd, }\DataTypeTok{newdata =}\NormalTok{ test)}
\NormalTok{pred <-}\StringTok{ }\NormalTok{qd.pred}\OperatorTok{$}\NormalTok{class}
\NormalTok{caret}\OperatorTok{::}\KeywordTok{confusionMatrix}\NormalTok{(pred, test}\OperatorTok{$}\NormalTok{taste)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{p.est <-qd.pred}\OperatorTok{$}\NormalTok{posterior}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pred <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(nb, }\DataTypeTok{newdata =}\NormalTok{ test)}
\NormalTok{caret}\OperatorTok{::}\KeywordTok{confusionMatrix}\NormalTok{(pred, test}\OperatorTok{$}\NormalTok{taste)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{rand.gen <-}\StringTok{ }\NormalTok{runif }\CommentTok{# rnorm}
\NormalTok{x1 <-}\StringTok{ }\KeywordTok{rand.gen}\NormalTok{(n)}
\NormalTok{rho <-}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(}\FloatTok{0.99}\NormalTok{) }\CommentTok{# coeficiente de correlación}
\NormalTok{x2 <-}\StringTok{ }\NormalTok{rho}\OperatorTok{*}\NormalTok{x1 }\OperatorTok{+}\StringTok{ }\KeywordTok{sqrt}\NormalTok{(}\DecValTok{1} \OperatorTok{-}\StringTok{ }\NormalTok{rho}\OperatorTok{^}\DecValTok{2}\NormalTok{)}\OperatorTok{*}\KeywordTok{rand.gen}\NormalTok{(n)}
\NormalTok{fit.x2 <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(x2 }\OperatorTok{~}\StringTok{ }\NormalTok{x1)}
\CommentTok{# plot(x1, x2)}
\CommentTok{# summary(fit.x2)}

\CommentTok{# Rejilla x-y para predicciones:}
\NormalTok{x1.range <-}\StringTok{ }\KeywordTok{range}\NormalTok{(x1)}
\NormalTok{x1.grid <-}\StringTok{ }\KeywordTok{seq}\NormalTok{(x1.range[}\DecValTok{1}\NormalTok{], x1.range[}\DecValTok{2}\NormalTok{], }\DataTypeTok{length.out =} \DecValTok{30}\NormalTok{)}
\NormalTok{x2.range <-}\StringTok{ }\KeywordTok{range}\NormalTok{(x2)}
\NormalTok{x2.grid <-}\StringTok{ }\KeywordTok{seq}\NormalTok{(x2.range[}\DecValTok{1}\NormalTok{], x2.range[}\DecValTok{2}\NormalTok{], }\DataTypeTok{length.out =} \DecValTok{30}\NormalTok{)}
\NormalTok{xy <-}\StringTok{ }\KeywordTok{expand.grid}\NormalTok{(}\DataTypeTok{x1 =}\NormalTok{ x1.grid, }\DataTypeTok{x2 =}\NormalTok{ x2.grid)}

\CommentTok{# Modelo teórico:}
\NormalTok{model.teor <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(x1, x2) x1}
\CommentTok{# model.teor <- function(x1, x2) x1 - 0.5*x2}
\NormalTok{y.grid <-}\StringTok{ }\KeywordTok{matrix}\NormalTok{(}\KeywordTok{mapply}\NormalTok{(model.teor, xy}\OperatorTok{$}\NormalTok{x1, xy}\OperatorTok{$}\NormalTok{x2), }\DataTypeTok{nrow =} \KeywordTok{length}\NormalTok{(x1.grid))}
...
\NormalTok{ylim <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\OperatorTok{-}\DecValTok{2}\NormalTok{, }\DecValTok{3}\NormalTok{) }\CommentTok{# range(y, y.pred)}
\KeywordTok{scatter3D}\NormalTok{(}\DataTypeTok{z =}\NormalTok{ y.mean, }\DataTypeTok{x =}\NormalTok{ x1, }\DataTypeTok{y =}\NormalTok{ x2, }\DataTypeTok{pch =} \DecValTok{16}\NormalTok{, }\DataTypeTok{cex =} \DecValTok{1}\NormalTok{, }\DataTypeTok{clim =}\NormalTok{ ylim, }\DataTypeTok{zlim =}\NormalTok{ ylim,}
          ...
          \DataTypeTok{main =} \StringTok{"Modelo teórico y valores de las variables explicativas"}\NormalTok{,}
          \DataTypeTok{xlab =} \StringTok{"x1"}\NormalTok{, }\DataTypeTok{ylab =} \StringTok{"x2"}\NormalTok{, }\DataTypeTok{zlab =} \StringTok{"y"}\NormalTok{, }\DataTypeTok{sub =} \KeywordTok{sprintf}\NormalTok{(}\StringTok{"R2(x1,x2) = %.2f"}\NormalTok{, }\KeywordTok{summary}\NormalTok{(fit.x2)}\OperatorTok{$}\NormalTok{r.squared),}
          \DataTypeTok{surf =} \KeywordTok{list}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ x1.grid, }\DataTypeTok{y =}\NormalTok{ x2.grid, }\DataTypeTok{z =}\NormalTok{ y.grid, }\DataTypeTok{facets =} \OtherTok{NA}\NormalTok{))}
\KeywordTok{scatter3D}\NormalTok{(}\DataTypeTok{z =} \KeywordTok{rep}\NormalTok{(ylim[}\DecValTok{1}\NormalTok{], n), }\DataTypeTok{x =}\NormalTok{ x1, }\DataTypeTok{y =}\NormalTok{ x2, }\DataTypeTok{add =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{colkey =} \OtherTok{FALSE}\NormalTok{, }
           \DataTypeTok{pch =} \DecValTok{16}\NormalTok{, }\DataTypeTok{cex =} \DecValTok{1}\NormalTok{, }\DataTypeTok{col =} \StringTok{"black"}\NormalTok{)}
\NormalTok{x2.pred <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(fit.x2, }\DataTypeTok{newdata =} \KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{x1 =}\NormalTok{ x1.range))}
\KeywordTok{lines3D}\NormalTok{(}\DataTypeTok{z =} \KeywordTok{rep}\NormalTok{(ylim[}\DecValTok{1}\NormalTok{], }\DecValTok{2}\NormalTok{), }\DataTypeTok{x =}\NormalTok{ x1.range, }\DataTypeTok{y =}\NormalTok{ x2.pred, }\DataTypeTok{add =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{colkey =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{col =} \StringTok{"black"}\NormalTok{) }
\end{Highlighting}
\end{Shaded}
...
\begin{center}\animategraphics[width=0.8\linewidth,loop]{1}{06-modelos_lineales_files/figure-latex/multicol-movie-}{1}{10}\end{center}
...
\begin{center}\animategraphics[width=0.8\linewidth,loop]{1}{06-modelos_lineales_files/figure-latex/indep-movie-}{1}{10}\end{center}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{summary}\NormalTok{(modelo)}\OperatorTok{$}\NormalTok{adj.r.squared}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{obs <-}\StringTok{ }\NormalTok{test}\OperatorTok{$}\NormalTok{fidelida}
\NormalTok{pred <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(modelo, }\DataTypeTok{newdata =}\NormalTok{ test)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{caret.lmStepAIC}\OperatorTok{$}\NormalTok{finalModel}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{with}\NormalTok{(caret.leapSeq, }\KeywordTok{coef}\NormalTok{(finalModel, bestTune}\OperatorTok{$}\NormalTok{nvmax))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{x <-}\StringTok{ }\KeywordTok{as.matrix}\NormalTok{(train[, }\DecValTok{-14}\NormalTok{])}
\NormalTok{y <-}\StringTok{ }\NormalTok{train}\OperatorTok{$}\NormalTok{fidelida}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cv.ridge}\OperatorTok{$}\NormalTok{lambda}\FloatTok{.1}\NormalTok{se}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(cv.lasso}\OperatorTok{$}\NormalTok{glmnet.fit, }\DataTypeTok{xvar =} \StringTok{"lambda"}\NormalTok{, }\DataTypeTok{label =} \OtherTok{TRUE}\NormalTok{)    }
\KeywordTok{abline}\NormalTok{(}\DataTypeTok{v =} \KeywordTok{log}\NormalTok{(cv.lasso}\OperatorTok{$}\NormalTok{lambda}\FloatTok{.1}\NormalTok{se), }\DataTypeTok{lty =} \DecValTok{2}\NormalTok{)}
\KeywordTok{abline}\NormalTok{(}\DataTypeTok{v =} \KeywordTok{log}\NormalTok{(cv.lasso}\OperatorTok{$}\NormalTok{lambda.min), }\DataTypeTok{lty =} \DecValTok{3}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{with}\NormalTok{(caret.pcrsel, }\KeywordTok{coef}\NormalTok{(finalModel, bestTune}\OperatorTok{$}\NormalTok{nvmax))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{df <-}\StringTok{ }\NormalTok{hbat[, }\KeywordTok{c}\NormalTok{(}\DecValTok{7}\OperatorTok{:}\DecValTok{19}\NormalTok{, }\DecValTok{24}\NormalTok{)]  }
\KeywordTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{nobs <-}\StringTok{ }\KeywordTok{nrow}\NormalTok{(df)}
\NormalTok{itrain <-}\StringTok{ }\KeywordTok{sample}\NormalTok{(nobs, }\FloatTok{0.8} \OperatorTok{*}\StringTok{ }\NormalTok{nobs)}
\NormalTok{train <-}\StringTok{ }\NormalTok{df[itrain, ]}
...
\KeywordTok{plot}\NormalTok{(train, }\DataTypeTok{pch =} \KeywordTok{as.numeric}\NormalTok{(train}\OperatorTok{$}\NormalTok{alianza), }\DataTypeTok{col =} \KeywordTok{as.numeric}\NormalTok{(train}\OperatorTok{$}\NormalTok{alianza))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{caret}\OperatorTok{::}\KeywordTok{confusionMatrix}\NormalTok{(pred, test}\OperatorTok{$}\NormalTok{alianza, }\DataTypeTok{positive =} \StringTok{"Si"}\NormalTok{, }\DataTypeTok{mode =} \StringTok{"everything"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{# Ajuste de los modelos}
\NormalTok{fit1 <-}\StringTok{ }\KeywordTok{knnreg}\NormalTok{(accel }\OperatorTok{~}\StringTok{ }\NormalTok{times, }\DataTypeTok{data =}\NormalTok{ mcycle, }\DataTypeTok{k =} \DecValTok{5}\NormalTok{) }\CommentTok{# 5 observaciones más cercanas (5% de los datos)}
\NormalTok{fit2 <-}\StringTok{ }\KeywordTok{knnreg}\NormalTok{(accel }\OperatorTok{~}\StringTok{ }\NormalTok{times, }\DataTypeTok{data =}\NormalTok{ mcycle, }\DataTypeTok{k =} \DecValTok{10}\NormalTok{)}
\NormalTok{fit3 <-}\StringTok{ }\KeywordTok{knnreg}\NormalTok{(accel }\OperatorTok{~}\StringTok{ }\NormalTok{times, }\DataTypeTok{data =}\NormalTok{ mcycle, }\DataTypeTok{k =} \DecValTok{20}\NormalTok{)}

\KeywordTok{plot}\NormalTok{(accel }\OperatorTok{~}\StringTok{ }\NormalTok{times, }\DataTypeTok{data =}\NormalTok{ mcycle, }\DataTypeTok{col =} \StringTok{'darkgray'}\NormalTok{) }
\NormalTok{newx <-}\StringTok{ }\KeywordTok{seq}\NormalTok{(}\DecValTok{1}\NormalTok{ , }\DecValTok{60}\NormalTok{, }\DataTypeTok{len =} \DecValTok{200}\NormalTok{)}
\NormalTok{newdata <-}\StringTok{ }\KeywordTok{data.frame}\NormalTok{(}\DataTypeTok{times =}\NormalTok{ newx)}
...
\KeywordTok{legend}\NormalTok{(}\StringTok{"topright"}\NormalTok{, }\DataTypeTok{legend =} \KeywordTok{c}\NormalTok{(}\StringTok{"5-NN"}\NormalTok{, }\StringTok{"10-NN"}\NormalTok{, }\StringTok{"20-NN"}\NormalTok{), }
       \DataTypeTok{lty =} \KeywordTok{c}\NormalTok{(}\DecValTok{3}\NormalTok{, }\DecValTok{2}\NormalTok{, }\DecValTok{1}\NormalTok{), }\DataTypeTok{lwd =} \DecValTok{1}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{y <-}\StringTok{ }\NormalTok{mcycle}\OperatorTok{$}\NormalTok{accel  }

\KeywordTok{library}\NormalTok{(KernSmooth)}
\NormalTok{h <-}\StringTok{ }\KeywordTok{dpill}\NormalTok{(x, y) }\CommentTok{# Método plug-in de Ruppert, Sheather y Wand (1995)}
\NormalTok{fit <-}\StringTok{ }\KeywordTok{locpoly}\NormalTok{(x, y, }\DataTypeTok{bandwidth =}\NormalTok{ h) }\CommentTok{# Estimación lineal local}
\KeywordTok{plot}\NormalTok{(x, y, }\DataTypeTok{col =} \StringTok{'darkgray'}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(fit)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pred <-}\StringTok{ }\KeywordTok{approx}\NormalTok{(fit, }\DataTypeTok{xout =}\NormalTok{ x)}\OperatorTok{$}\NormalTok{y }\CommentTok{# pred <- predict(fit)}
\NormalTok{resid <-}\StringTok{ }\NormalTok{y }\OperatorTok{-}\StringTok{ }\NormalTok{pred }\CommentTok{# resid <- residuals(fit)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{  ))}
\NormalTok{\}}
\KeywordTok{accuracy}\NormalTok{(pred, y)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{cv.loess <-}\StringTok{ }\ControlFlowTok{function}\NormalTok{(formula, datos, span, ...) \{}
\NormalTok{  n <-}\StringTok{ }\KeywordTok{nrow}\NormalTok{(datos)}
\NormalTok{  cv.pred <-}\StringTok{ }\KeywordTok{numeric}\NormalTok{(n)}
  \ControlFlowTok{for}\NormalTok{ (i }\ControlFlowTok{in} \DecValTok{1}\OperatorTok{:}\NormalTok{n) \{}
\NormalTok{    modelo <-}\StringTok{ }\KeywordTok{loess}\NormalTok{(formula, datos[}\OperatorTok{-}\NormalTok{i, ], }\DataTypeTok{span =}\NormalTok{ span, }
...
\NormalTok{ventanas <-}\StringTok{ }\KeywordTok{seq}\NormalTok{(}\FloatTok{0.1}\NormalTok{, }\FloatTok{0.5}\NormalTok{, }\DataTypeTok{len =} \DecValTok{10}\NormalTok{)}
...
\NormalTok{  cv.pred <-}\StringTok{ }\KeywordTok{cv.loess}\NormalTok{(accel }\OperatorTok{~}\StringTok{ }\NormalTok{times, mcycle, ventanas[p], }\DataTypeTok{family =} \StringTok{"symmetric"}\NormalTok{)}
  \CommentTok{# cv.error[p] <- mean((cv.pred - mcycle$accel)^2)}
\NormalTok{  cv.error[p] <-}\StringTok{ }\KeywordTok{median}\NormalTok{(}\KeywordTok{abs}\NormalTok{(cv.pred }\OperatorTok{-}\StringTok{ }\NormalTok{mcycle}\OperatorTok{$}\NormalTok{accel))}
\NormalTok{\}}

\KeywordTok{plot}\NormalTok{(ventanas, cv.error)}
\NormalTok{imin <-}\StringTok{ }\KeywordTok{which.min}\NormalTok{(cv.error)}
\NormalTok{span.cv <-}\StringTok{ }\NormalTok{ventanas[imin]}
\KeywordTok{points}\NormalTok{(span.cv, cv.error[imin], }\DataTypeTok{pch =} \DecValTok{16}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{plot}\NormalTok{(accel }\OperatorTok{~}\StringTok{ }\NormalTok{times, }\DataTypeTok{data =}\NormalTok{ mcycle, }\DataTypeTok{col =} \StringTok{'darkgray'}\NormalTok{)}
\NormalTok{fit <-}\StringTok{ }\KeywordTok{loess}\NormalTok{(accel }\OperatorTok{~}\StringTok{ }\NormalTok{times, mcycle, }\DataTypeTok{span =}\NormalTok{ span.cv, }\DataTypeTok{family =} \StringTok{"symmetric"}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(mcycle}\OperatorTok{$}\NormalTok{times, }\KeywordTok{predict}\NormalTok{(fit))}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(accel }\OperatorTok{~}\StringTok{ }\NormalTok{times, }\DataTypeTok{data =}\NormalTok{ mcycle, }\DataTypeTok{col =} \StringTok{'darkgray'}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(mcycle}\OperatorTok{$}\NormalTok{times, }\KeywordTok{predict}\NormalTok{(mars))}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(accel }\OperatorTok{~}\StringTok{ }\NormalTok{times, }\DataTypeTok{data =}\NormalTok{ mcycle, }\DataTypeTok{col =} \StringTok{'darkgray'}\NormalTok{)}
\KeywordTok{lines}\NormalTok{(mcycle}\OperatorTok{$}\NormalTok{times, }\KeywordTok{predict}\NormalTok{(mars2))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{summary}\NormalTok{(caret.mars}\OperatorTok{$}\NormalTok{finalModel)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# plotmo(caret.mars$finalModel, caption = 'ozone$O3 (caret "earth" method)')}
\KeywordTok{plotmo}\NormalTok{(caret.mars}\OperatorTok{$}\NormalTok{finalModel, }\DataTypeTok{degree2 =} \DecValTok{0}\NormalTok{, }\DataTypeTok{caption =} \StringTok{'ozone$O3 (efectos principales)'}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plotmo}\NormalTok{(caret.mars}\OperatorTok{$}\NormalTok{finalModel, }\DataTypeTok{degree1 =} \DecValTok{0}\NormalTok{, }\DataTypeTok{caption =} \StringTok{'ozone$O3 (interacciones)'}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pred <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(caret.mars, }\DataTypeTok{newdata =}\NormalTok{ test)}
\KeywordTok{accuracy}\NormalTok{(pred, test}\OperatorTok{$}\NormalTok{O3)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pred <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(ppreg, }\DataTypeTok{newdata =}\NormalTok{ test)}
\NormalTok{obs <-}\StringTok{ }\NormalTok{test}\OperatorTok{$}\NormalTok{O3}
\KeywordTok{plot}\NormalTok{(pred, obs, }\DataTypeTok{main =} \StringTok{"Observado frente a predicciones"}\NormalTok{,}
     \DataTypeTok{xlab =} \StringTok{"Predicción", ylab = "}\NormalTok{Observado}\StringTok{")}
\StringTok{abline(a = 0, b = 1)}
\StringTok{abline(lm(obs ~ pred), lty = 2)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{summary}\NormalTok{(caret.ppr}\OperatorTok{$}\NormalTok{finalModel)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{plot}\NormalTok{(caret.ppr}\OperatorTok{$}\NormalTok{finalModel)}
\end{Highlighting}
\end{Shaded}
...
\begin{center}\includegraphics[width=0.8\linewidth]{07-regresion_np_files/figure-latex/unnamed-chunk-42-1} \end{center}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{summary}\NormalTok{(caret.nnet}\OperatorTok{$}\NormalTok{finalModel)}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{plotnet}\NormalTok{(caret.nnet}\OperatorTok{$}\NormalTok{finalModel)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pred <-}\StringTok{ }\KeywordTok{predict}\NormalTok{(caret.nnet, }\DataTypeTok{newdata =}\NormalTok{ test)}
\NormalTok{obs <-}\StringTok{ }\NormalTok{test}\OperatorTok{$}\NormalTok{O3}

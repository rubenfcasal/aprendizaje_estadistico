...
\FunctionTok{lines}\NormalTok{(newdata}\SpecialCharTok{$}\NormalTok{lstat, }\FunctionTok{predict}\NormalTok{(fit.min, }\AttributeTok{newdata =}\NormalTok{ newdata))}
...
\begin{longtable}[]{@{}
  >{\centering\arraybackslash}p{(\columnwidth - 4\tabcolsep) * \real{0.2987}}
...
\NormalTok{datos}\SpecialCharTok{$}\NormalTok{fmedv }\OtherTok{\textless{}{-}} \FunctionTok{factor}\NormalTok{(datos}\SpecialCharTok{$}\NormalTok{medv }\SpecialCharTok{\textgreater{}} \DecValTok{25}\NormalTok{,             }\CommentTok{\# levels = c(\textquotesingle{}FALSE\textquotesingle{}, \textquotesingle{}TRUE\textquotesingle{})}
                      \AttributeTok{labels =} \FunctionTok{c}\NormalTok{(}\StringTok{"Bajo"}\NormalTok{, }\StringTok{"Alto"}\NormalTok{)) }
...
\NormalTok{pred }\OtherTok{\textless{}{-}} \FunctionTok{factor}\NormalTok{(p.est }\SpecialCharTok{\textgreater{}} \FloatTok{0.5}\NormalTok{, }\AttributeTok{labels =} \FunctionTok{c}\NormalTok{(}\StringTok{"Bajo"}\NormalTok{, }\StringTok{"Alto"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{f }\OtherTok{\textless{}{-}} \FunctionTok{factor}\NormalTok{(levels, }\AttributeTok{levels =}\NormalTok{ levels) }
\CommentTok{\# Nota: Al estar por orden alfabético se podría haber empleado factor(levels)}
...
\item
  ...
  \item
    ...
    \end{itemize}
  \item
    Dispone de múltiples funciones auxiliares, como \texttt{dummyVars()} o \texttt{rfe()} (\emph{recursive feature elimination}).
  \end{itemize}
\item
  ...
  \item
    ...
    \begin{itemize}
    \item
      \texttt{trControl}: permite establecer el método de remuestreo para la evaluación de los hiperparámetros y el método para seleccionar el óptimo, incluyendo las medidas de precisión.
      Por ejemplo \texttt{trControl\ =\ trainControl(method\ =\ "cv",\ number\ =\ 10,\ selectionFunction\ =\ "oneSE")}.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{barplot}\NormalTok{(}\FunctionTok{table}\NormalTok{(winequality}\SpecialCharTok{$}\NormalTok{quality), }\AttributeTok{xlab =} \StringTok{"Calidad"}\NormalTok{, }\AttributeTok{ylab =} \StringTok{"Frecuencia"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# plot(pred, obs, xlab = "Predicción", ylab = "Observado")}
...
\NormalTok{winetaste}\SpecialCharTok{$}\NormalTok{taste }\OtherTok{\textless{}{-}} \FunctionTok{factor}\NormalTok{(winequality}\SpecialCharTok{$}\NormalTok{quality }\SpecialCharTok{\textless{}} \DecValTok{6}\NormalTok{, }\CommentTok{\# levels = c(\textquotesingle{}FALSE\textquotesingle{}, \textquotesingle{}TRUE\textquotesingle{})}
                          \AttributeTok{labels =} \FunctionTok{c}\NormalTok{(}\StringTok{\textquotesingle{}good\textquotesingle{}}\NormalTok{, }\StringTok{\textquotesingle{}bad\textquotesingle{}}\NormalTok{)) }
...
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{caret.svm }\OtherTok{\textless{}{-}} \FunctionTok{train}\NormalTok{(taste }\SpecialCharTok{\textasciitilde{}}\NormalTok{ ., }\AttributeTok{data =}\NormalTok{ train, }\AttributeTok{method =} \StringTok{"svmRadial"}\NormalTok{, }
...
\CommentTok{\# p.est \textless{}{-} qd.pred$posterior}
\NormalTok{caret}\SpecialCharTok{::}\FunctionTok{confusionMatrix}\NormalTok{(pred, test}\SpecialCharTok{$}\NormalTok{taste)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{\# p.est \textless{}{-} predict(nb, newdata = test, type = "raw")}
\NormalTok{caret}\SpecialCharTok{::}\FunctionTok{confusionMatrix}\NormalTok{(pred, test}\SpecialCharTok{$}\NormalTok{taste)}
\end{Highlighting}
\end{Shaded}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.2154}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 2\tabcolsep) * \real{0.7846}}@{}}
...
\FunctionTok{plot}\NormalTok{(pred, obs, }\AttributeTok{xlab =} \StringTok{"Predicción"}\NormalTok{, }\AttributeTok{ylab =} \StringTok{"Observado"}\NormalTok{)}
\FunctionTok{abline}\NormalTok{(}\AttributeTok{a =} \DecValTok{0}\NormalTok{, }\AttributeTok{b =} \DecValTok{1}\NormalTok{)}
\NormalTok{res }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(obs }\SpecialCharTok{\textasciitilde{}}\NormalTok{ pred) }\CommentTok{\# summary(res)}
\FunctionTok{abline}\NormalTok{(res, }\AttributeTok{lty =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\FunctionTok{load}\NormalTok{(}\StringTok{"data/hbat.RData"}\NormalTok{)}
\NormalTok{df }\OtherTok{\textless{}{-}}\NormalTok{ hbat[}\FunctionTok{c}\NormalTok{(}\DecValTok{23}\NormalTok{, }\DecValTok{7}\SpecialCharTok{:}\DecValTok{19}\NormalTok{)]}
\FunctionTok{set.seed}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\NormalTok{nobs }\OtherTok{\textless{}{-}} \FunctionTok{nrow}\NormalTok{(df)}
\NormalTok{itrain }\OtherTok{\textless{}{-}} \FunctionTok{sample}\NormalTok{(nobs, }\FloatTok{0.8} \SpecialCharTok{*}\NormalTok{ nobs)}
\NormalTok{train }\OtherTok{\textless{}{-}}\NormalTok{ df[itrain, ]}
\NormalTok{test }\OtherTok{\textless{}{-}}\NormalTok{ df[}\SpecialCharTok{{-}}\NormalTok{itrain, ]}
\NormalTok{x }\OtherTok{\textless{}{-}} \FunctionTok{as.matrix}\NormalTok{(train[}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{])}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{plot}\NormalTok{(train[}\SpecialCharTok{{-}}\DecValTok{1}\NormalTok{], }\AttributeTok{pch =} \FunctionTok{as.numeric}\NormalTok{(train}\SpecialCharTok{$}\NormalTok{alianza), }\AttributeTok{col =} \FunctionTok{as.numeric}\NormalTok{(train}\SpecialCharTok{$}\NormalTok{alianza))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{times }\OtherTok{\textless{}{-}}\NormalTok{ mcycle}\SpecialCharTok{$}\NormalTok{times}
\NormalTok{accel }\OtherTok{\textless{}{-}}\NormalTok{ mcycle}\SpecialCharTok{$}\NormalTok{accel  }
\FunctionTok{library}\NormalTok{(KernSmooth)}
\NormalTok{h }\OtherTok{\textless{}{-}} \FunctionTok{dpill}\NormalTok{(times, accel) }\CommentTok{\# Método plug{-}in de Ruppert, Sheather y Wand (1995)}
\NormalTok{fit }\OtherTok{\textless{}{-}} \FunctionTok{locpoly}\NormalTok{(times, accel, }\AttributeTok{bandwidth =}\NormalTok{ h) }\CommentTok{\# Estimación lineal local}
...
\caption{Ajuste lineal local con ventana plug-in.}\label{fig:llr-fit}
\end{figure}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{pred }\OtherTok{\textless{}{-}} \FunctionTok{approx}\NormalTok{(fit, }\AttributeTok{xout =}\NormalTok{ times)}\SpecialCharTok{$}\NormalTok{y }\CommentTok{\# pred \textless{}{-} predict(fit)}
\NormalTok{resid }\OtherTok{\textless{}{-}}\NormalTok{ accel }\SpecialCharTok{{-}}\NormalTok{ pred }\CommentTok{\# resid \textless{}{-} residuals(fit)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{  ))}
\NormalTok{\}}
\FunctionTok{accuracy}\NormalTok{(pred, accel)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# plotmo(caret.mars$finalModel)}
\FunctionTok{plotmo}\NormalTok{(caret.mars}\SpecialCharTok{$}\NormalTok{finalModel, }\AttributeTok{degree2 =} \DecValTok{0}\NormalTok{, }\AttributeTok{caption =} \StringTok{""}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{plotmo}\NormalTok{(caret.mars}\SpecialCharTok{$}\NormalTok{finalModel, }\AttributeTok{degree1 =} \DecValTok{0}\NormalTok{, }\AttributeTok{caption =} \StringTok{""}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Para ajustar un modelo \emph{single index} también se podría emplear la función \href{https://rdrr.io/pkg/np/man/np.singleindex.html}{\texttt{npindex()}} del paquete \href{https://github.com/JeffreyRacine/R-Package-np}{\texttt{np}} (que implementa el método de \protect\hyperlink{ref-ichimura1993}{Ichimura, 1993}, considerando un estimador local constante), aunque en este caso ni el tiempo de computación ni el resultado es satisfactorio\footnote{No admite una fórmula del tipo \texttt{respuesta\ \textasciitilde{}\ .}:

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{bw }\OtherTok{\textless{}{-}} \FunctionTok{npindexbw}\NormalTok{(O3 }\SpecialCharTok{\textasciitilde{}}\NormalTok{ ., }\AttributeTok{data =}\NormalTok{ train)}
...
\NormalTok{formula }\OtherTok{\textless{}{-}} \FunctionTok{reformulate}\NormalTok{(}\FunctionTok{setdiff}\NormalTok{(}\FunctionTok{colnames}\NormalTok{(train), }\StringTok{"O3"}\NormalTok{), }\AttributeTok{response =} \StringTok{"O3"}\NormalTok{) }\CommentTok{\# Escribe la formula explícitamente}
\end{Highlighting}
\end{Shaded}

  El valor por defecto de \texttt{nmulti\ =\ 5} (número de reinicios con punto de partida aleatorio del algoritmo de optimización) incrementa el tiempo de computación.
...
\begin{itemize}
\item
  \texttt{formula} y \texttt{data} (opcional): permiten especificar la respuesta y las variables predictoras de la forma habitual (e.g.~\texttt{respuesta\ \textasciitilde{}\ .}; también implementa una interfaz con matrices \texttt{x} e \texttt{y}).
  Admite respuestas multidimensionales (ajustará un modelo para cada componente) y categóricas (las convierte en multivariantes si tienen más de dos categorías y emplea \emph{softmax} en los nodos finales).
...
\item
  \texttt{decay}: parámetro \(\lambda\) de regularización de los pesos (\emph{weight decay}); por defecto 0.
...
\NormalTok{old.par }\OtherTok{\textless{}{-}} \FunctionTok{par}\NormalTok{(}\AttributeTok{mar =} \FunctionTok{c}\NormalTok{(}\AttributeTok{bottom =} \DecValTok{1}\NormalTok{, }\AttributeTok{left =} \DecValTok{2}\NormalTok{, }\AttributeTok{top =} \DecValTok{2}\NormalTok{, }\AttributeTok{right =} \DecValTok{3}\NormalTok{), }\AttributeTok{xpd =} \ConstantTok{NA}\NormalTok{)}
...
\leavevmode\vadjust pre{\hypertarget{ref-agor2019feature}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-becker2021mlr3}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-bellman1961adaptive}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-chollet2018deep}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-cortez2009modeling}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-craven1978smoothing}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-culp2006ada}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-de1978practical}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-dietterich2000experimental}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-drucker1997support}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-dunson2018statistics}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-freund1996schapire}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-friedman1989regularized}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-friedman1991multivariate}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-friedman2002stochastic}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-friedman2008predictive}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-friedman1981projection}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-goldstein2015peeking}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-greenwell2020dblp}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-hair1998multivariate}{}}%
Hair, J. F., Anderson, R. E., Tatham, R. L., y Black, W. (1998). \emph{Multivariate Data Analysis}. Prentice Hall.

\leavevmode\vadjust pre{\hypertarget{ref-hastie2017generalized}{}}%
Hastie, T. J., y Pregibon, D. (2017). \emph{Generalized linear models} (pp. 195-247). Routledge.

\leavevmode\vadjust pre{\hypertarget{ref-hastie2004entire}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-hastie1996fisher}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-hothorn2006unbiased}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-ichimura1993}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-james2021introduction}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-kernlab2004}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-kass1980exploratory}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-kearns_cryptographic_1994}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-kruskal1969toward}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-kuhn2008building}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-kuhn2022tidy}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-kvaalseth1985cautionary}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-lauro1996computational}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-liaw2002classification}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-mcculloch1943logical}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-Mevik2007pls}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-quinlan1992learning}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-spinoza1667ethics}{}}%
Spinoza, B. (1677). \emph{Ethics}.

\leavevmode\vadjust pre{\hypertarget{ref-strumbelj2010efficient}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-valiant1984theory}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-vapnik1998}{}}%
Vapnik, V. (2000). \emph{Statistical Learning Theory}. Willey.

\leavevmode\vadjust pre{\hypertarget{ref-vapnik2013nature}{}}%
Vapnik, V. (2013). \emph{The Nature of Statistical Learning Theory}. Springer.

\leavevmode\vadjust pre{\hypertarget{ref-Venables2002Modern}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-vinayak2015dart}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-R-KernSmooth}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-R-rattle}{}}%
Williams, G. (2022). \emph{rattle: Graphical User Interface for Data Science in R}. \url{https://rattle.togaware.com/}

\leavevmode\vadjust pre{\hypertarget{ref-wold1983multivariate}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-wolpert1997no}{}}%

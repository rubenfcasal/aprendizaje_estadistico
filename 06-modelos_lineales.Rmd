...
En R podemos emplear la función `r cite_fun(lm, stats)`:
...
Vemos también que, si consideramos un modelo de regresión lineal simple, el mejor ajuste se obtendría empleando `velocida` como variable explicativa `r cite_fig(lm1)`:
...
Para calcular predicciones (estimaciones de la media condicionada), también intervalos de confianza o de predicción, se puede emplear el correspondiente método `predict()` (consultar la ayuda de `r cite_fun(predict.lm)` para ver todas las opciones disponibles).
...
```{r lm3d, fig.cap='(ref:lm3d)', message=FALSE, warning=FALSE}
library(plot3D)
...
```{r multicol-movie, echo=TRUE, eval=is_html(), include=FALSE, fig.show='animate', interval=1, aniopts="loop", ffmpeg.format='gif', fig.path="images/"}
sd.err <- 0.25
nsim <- 10
...
```{r indep-movie, echo=TRUE, eval=is_html(), include=FALSE, fig.show='animate', interval=1, aniopts="loop", ffmpeg.format='gif', fig.path="images/"}
x2 <- rand.gen(n)
y.mean <- mapply(model.teor, x1, x2)
...
`vif()` del paquete `r cite_cran(car)` para calcular los factores de inflación de varianza para las variables del modelo.
...
```{r regsel,  fig.cap="Modelos obtenidos mediante búsqueda exhaustiva ordenados según su coeficiente de determinación ajustado."}
...
La función `r cite_fun(stepAIC, MASS)` del paquete `r cite_cran(MASS)` permite seleccionar el modelo por pasos^[También está disponible la función `step()` del paquete base `stats` con menos opciones.], hacia delante o hacia atrás según criterio AIC (*Akaike Information Criterion*) o BIC (*Bayesian Information Criterion*). 
...
```{r lm-plot, out.width = "90%", fig.dim = c(10, 10), fig.cap="Gráficos de diagnóstico del ajuste lineal."}
oldpar <- par(mfrow = c(2,2))
plot(modelo)
par(oldpar)
```
...
El tercer gráfico de dispersión-nivel permite detectar heterocedasticidad (la pendiente debería ser nula) y ayudar a seleccionar una transformación para corregirla (también se podría emplear la función `r cite_fun(boxcox, MASS)` del paquete `r cite_cran(MASS)`).
...
pero puede ser recomendable emplear regresión lineal robusta, por ejemplo mediante la función `r cite_fun(rlm, MASS)` del paquete `r cite_cran(MASS)`).
...
Aunque puede ser preferible emplear las funciones `r cite_fun(crPlots, car)` ó `r cite_fun(avPlots, car)` del paquete `r cite_cran(car)`^[Estas funciones permitirían además detectar puntos atípicos o influyentes 
...
implementado en el paquete `r cite_cran(mctest)`.
La librería `r cite_cran(lmtest)` proporciona herramientas adicionales para la diagnosis de modelos lineales, 
...
Podemos evaluar el modelo ajustado en el conjunto de datos de test y comparar las predicciones frente a los valores reales `r cite_fig(lm-test)`:
...
Hay varios paquetes que implementan estos métodos: `h2o`, `elasticnet`, `penalized`, `lasso2`, `biglasso`, etc., pero el paquete `r cite_pkg(glmnet)` utiliza una de las más eficientes.
...
Se puede emplear la función `r cite_fun(model.matrix)` (o `r cite_fun(Matrix::sparse.model.matrix)` si el conjunto de datos es muy grande) para construir la matriz de diseño `x` a partir de una fórmula (alternativamente se pueden emplear la herramientas implementadas en el paquete `caret`). 
...
Aunque para seleccionar el un valor "óptimo" del hiperparámetro $\lambda$ (mediante validación cruzada) se puede emplear `r cite_fun(cv.glmnet, glmnet)`:
...
```{r elastic-caret, fig.dim=c(9, 7), fig.cap='(ref:elastic-caret)'}
...
Aunque podríamos emplear las funciones `printcomp()` y `lm()` del paquete base, emplearemos por comodidad la función `r cite_fun(pcr, pls)` del paquete `r cite_pkg_("pls","https://mevik.net/work/software/pls.html")`, ya que incorpora validación cruzada para seleccionar el número de componentes y facilita el cálculo de nuevas predicciones.
...
En este caso, empleando el criterio de menor error de validación cruzada se seleccionaría un número elevado de componentes, el mínimo se alcanzaría con `r ncomp.op` componentes (bastante próximo a ajustar un modelo lineal con todos los predictores).
...
```{r pcr2-plot-caret, fig.cap="Errores de validación cruzada en función del número de componentes en el ajuste mediante PCR con selección por pasos y valor óptimo según la regla de un error estándar."}
set.seed(1)
...
En este caso el mínimo se alcanza con `r ncomp.op` componentes pero, a la vista de la Figura \@ref(fig:plsr-plot), parece que 4 sería un valor razonable.
...
```{r plsr-plot-caret, fig.cap="Errores de validación cruzada en función del número de componentes en el ajuste mediante PLS."}
modelLookup("pls")
set.seed(1)
...
Para el ajuste (estimación de los parámetros) de un modelo lineal generalizado a un conjunto de datos (por máxima verosimilitud) se emplea la función `r cite_fun(glm, stats)` (la mayoría de los principales parámetros coinciden con los de la función `lm()`):
...
También se podría emplear la función `bigglm()` del paquete `r cite_cran(biglm)` para ajustar modelos lineales generalizados a grandes conjuntos de datos, aunque en este caso los requerimientos computacionales pueden ser mayores.
...
Por ejemplo, si el número de variables no es muy grande, podemos generar un gráfico de dispersión matricial, diferenciando las observaciones pertenecientes a las distintas clases `r cite_fig(plot-df-class)`.
...
También se podría emplear la función `r cite_fun(stepwise, RcmdrMisc)` del paquete `r cite_cran(RcmdrMisc)` para seleccionar un modelo por pasos según criterio AIC o BIC:
...
```{r glm-plot,out.width = "90%", fig.dim = c(10, 10), fig.cap="Gráficos de diagnóstico del ajuste lineal generalizado."}
oldpar <- par( mfrow=c(2,2))
plot(modelo)
par(oldpar)
```
...
```{r crPlots-glm, out.width = "90%", fig.dim = c(10,10), fig.cap= "Gráficos parciales de residuos del ajuste generalizado."}
# library(car)

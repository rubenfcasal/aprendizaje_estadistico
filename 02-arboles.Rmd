...
Al final de este proceso iterativo el espacio predictor se ha particionado en regiones de forma rectangular en la que la predicción de la respuesta es constante  `r cite_fig(predictor)`. 
...
La metodología CART está implementada en el paquete `r cite_cran(rpart)` 
(Recursive PARTitioning)^[El paquete `r cite_cran(tree)` es una traducción del original en S.]. 
...
Para representarlo se puede emplear las herramientas del paquete `r cite_cran("rpart")`  `r cite_fig(arbolrpart)`:
...
```{r arbolrpart, fig.cap='(ref:arbolrpart)'}
plot(tree)
text(tree)
```

Pero puede ser preferible emplear el paquete `r cite_cran("rpart.plot")`  `r cite_fig(arbolrpartplot)`:
...
Posteriormente podemos emplear las funciones `r cite_fun(printcp, rpart)` (o `r cite_fun(plotcp, rpart)`) para obtener (representar) 
...
El último paso sería evaluarlo en la muestra de test siguiendo los pasos descritos en la Sección \@ref(eval-reg) `r cite_fig(obsXpred)`:
...
Para ello se puede modificar la información que se muestra en cada nodo `r cite_fig(arbolextra)`:
...
```{r arbolclassifpoda, fig.cap='(ref:arbolclassifpoda)'}
xerror <- tree$cptable[,"xerror"]
imin.xerror <- which.min(xerror)
upper.xerror <- xerror[imin.xerror] + tree$cptable[imin.xerror, "xstd"]
icp <- min(which(xerror <= upper.xerror))
...
El método `r cite_fun(predict.rpart, rpart)` devuelve por defecto (`type = "prob"`) una matriz con las probabilidades de cada clase, por lo que habrá que establecer `type = "class"` (para más detalles consultar la ayuda de esta función).
...
Si queremos emplear validación cruzada como en el caso anterior podemos emplear la función auxiliar `r cite_fun(trainControl, caret)` y para considerar un mayor rango de posibles valores, el argumento `tuneLength` `r cite_fig(arbolclassifggplot)`.
...
```{r arbolfinalcaret, fig.cap='(ref:arbolfinalcaret)'}
caret.rpart$finalModel
rpart.plot(caret.rpart$finalModel)
```

Para utilizar la regla de "un error estándar" se puede añadir `selectionFunction = "oneSE"` en las opciones de entrenamiento[^rpart-1]`r cite_fig(arbolclassifoneSE)`:
...
```{r arbolclassifoneSE, fig.cap='(ref:arbolclassifoneSE)'}
set.seed(1)
caret.rpart <- train(taste ~ ., method = "rpart", data = train, tuneLength = 20,
                     trControl = trainControl(method = "cv", number = 10,
...
Para calcular las predicciones (o las estimaciones de las probabilidades) podemos emplear el método `r cite_fun(predict.train, caret)` y posteriormente `r cite_fun(confusionMatrix, caret)` para evaluar su precisión:
...
Esta metodología está implementada en el paquete `r cite_pkg_("C50", "https://topepo.github.io/C5.0/index.html")`. 
...
El paquete `r cite_pkg_("Cubist", "https://topepo.github.io/Cubist")` es una evolución de M5 que incorpora un método *ensemble* similar a *boosting*.
...
Una de las más utilizadas son los *conditional inference trees* [@hothorn2006unbiased]^[Otra alternativa es GUIDE (Generalized, Unbiased, Interaction Detection and Estimation; @loh2002regression).], implementada en la función `r cite_fun(ctree, party)` del paquete `r cite_cran("party")`.
...
```{r ctree-plot, fig.cap='(ref:ctree-plot)', fig.height=8, fig.width=10}
library(party)

...
```{r bagging-conv,fig.cap="Tasas de error OOB al usar bagging para la predicción de `winetaste$taste` (realizado empleando `randomForest()` con `mtry` igual al número de predictores)."}
plot(bagtrees, main = "")
...
```{r rf-plot,fig.cap="Tasas de error OOB al usar bosques aleatorios para la predicción de `winetaste$taste` (empleando `randomForest()` con las opciones por defecto)."}
plot(rf,main="")
legend("right", colnames(rf$err.rate), lty = 1:5, col = 1:6)
```

Podemos mostrar la importancia de las variables predictoras (utilizadas en el bosque aleatorio y sus sustituas) con la función `importance()` o representarlas con `varImpPlot()` `r citefig("rf-importance")`:

```{r rf-importance,fig.cap="Importancia de las variables predictoras al emplear bosques aleatorios para la predicción de `winetaste$taste`."}
importance(rf)
varImpPlot(rf)
```
...
Por ejemplo, empleando la función `pdp::partial()`, podemos generar gráficos PDP estimando los efectos individuales de los predictores `r citefig("rf-pdp-uni-plot")`:
...
```{r rf-pdp-uni-plot, echo=FALSE, out.width="90%", fig.path="images/", dev="png", fig.cap="Efecto parcial del alcochol (panel izquierdo) y la densidad (panel derecho) sobre la respuesta."}
library(pdp) # por si no se ejecuta el código anterior
knitr::include_graphics("images/rf-pdp-uni-1.png")
```

O gráficos PDP considerando la interacción entre dos predictores `r citefig("rf-pdp-plot")` (cuidado, puede requerir de mucho tiempo de computación):

```{r rf-pdp, eval=.regerar}
pdp12 <- partial(rf, c("alcohol", "density"))
plotPartial(pdp12)
```

```{r rf-pdp-plot, echo=FALSE, out.width="80%", fig.path="images/", dev="png", fig.cap="Efecto parcial de la interacción del alcochol y la densidad sobre la respuesta."}
...
```{r rf-ice-plot, echo=FALSE, out.width="80%", fig.cap="Efecto individual de cada observación de alcochol (panel izquierdo) y densidad (panel derecho) sobre la respuesta."}
...
```{r rf-vivid-plot, echo=FALSE, out.width="80%", fig.cap="Mapa de calor para la importancia e interaciones del ajuste de un bosque aleatorio usando vivid."}
...
```{r rf-vivid2-plot, echo=FALSE, out.width="80%", fig.cap="Gráfico de red para la importancia e interaciones del ajuste de un bosque aleatorio usando vivid."}
...
Con las opciones por defecto únicamente evalúa tres valores posibles del hiperparámetro `r citefig("rf-caret-train")`.
...
```{r rf-caret-train, fig.cap="Evolución de la precisión de un bosque aleatorio dependiendo del número de predictores seleccionados."}
set.seed(1)
rf.caret <- train(taste ~ ., data = train, method = "rf")
plot(rf.caret)
```

@breiman2001random sugiere emplear el valor por defecto para `mtry`, la mitad y el doble `r citefig("rf-caret-grid")`:

```{r rf-caret-grid, fig.cap="Evolución de la precisión de un bosque aleatorio con `caret` usando el argumento `tuneGrid`."}
...
La función `ada()` del paquete `r citepkg("ada")` [@culp2006ada] implementa diversos métodos boosting (incluyendo el algoritmo original AdaBoost). 
...
Con el método `plot()` podemos representar la evolución del error de clasificación al aumentar el número de iteraciones `r citefig("ada-plot")`:

```{r ada-plot, fig.cap="Evolución de la tasa de error utilizando `ada()`."}
plot(ada.boost)
```
...
El paquete `r citepkg("gbm")` implementa el algoritmo SGB de @friedman2002stochastic y admite varios tipos de respuesta considerando distintas funciones de pérdida (aunque en el caso de variables dicotómicas éstas deben tomar valores en $\{0, 1\}$^[Se puede evitar este inconveniente empleando la interfaz de `caret`.]).
...
El método `summary()` calcula las medidas de influencia de los predictores y las representa gráficamente `r citefig("gbm-summary")`:

```{r gbm-summary,fig.cap="Importancia de las variables predictoras (con los valores por defecto de `gbm()`)."}
summary(gbm.fit)
```

Para estudiar el efecto de un predictor se pueden generar gráficos de los efectos parciales mediante el método `plot()` `r citefig("gbm-plot")`:

```{r gbm-plot, fig.dim = c(12, 6), out.width = "90%", fig.cap="Efecto parcíal del alcohol (panel izquierdo) y la densidad (panel derecho) sobre la respuesta (con `gbm()`)."}

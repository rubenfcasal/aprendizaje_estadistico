...
Estos algoritmos son de los más populares en AE y están implementados en numerosos paquetes de R, aunque la referencia es el paquete `r cite_cran(randomForest)` (que emplea el código Fortran desarrollado por Leo Breiman y Adele Cutler).
...
Entre las numerosas alternativas, además de las implementadas en paquetes que integran colecciones de métodos como `h2o` o `RWeka`, una de las más utilizadas son los bosques aleatorios con *conditional inference trees*, implementada en la función `cforest()` del paquete `r cite_cran(party)`. 
...
Podemos mostrar la importancia de las variables predictoras (utilizadas en el bosque aleatorio y sus sustituas) con la función `importance()` o representarlas con `varImpPlot()` `r cite_fig(rf-importance)`:
...
Por ejemplo, empleando la función `pdp::partial()`, podemos generar gráficos PDP estimando los efectos individuales de los predictores `r cite_fig(rf-pdp-uni-plot)`:
...
```{r rf-pdp-uni-plot, echo=FALSE, out.width="90%", fig.path="images/", dev="png", fig.cap="Efecto parcial del alcohol (panel izquierdo) y la densidad (panel derecho) sobre la respuesta."}
library(pdp) # por si no se ejecuta el código anterior
knitr::include_graphics("images/rf-pdp-uni-1.png")
```

O gráficos PDP considerando la interacción entre dos predictores `r cite_fig(rf-pdp-plot)` (cuidado, puede requerir de mucho tiempo de computación):
...
```{r rf-pdp-plot, echo=FALSE, out.width="80%", fig.path="images/", dev="png", fig.cap="Efecto parcial de la interacción del alcohol y la densidad sobre la respuesta."}
...
```{r rf-ice-plot, echo=FALSE, out.width="90%", fig.cap="Efecto individual de cada observación de alcohol (panel izquierdo) y densidad (panel derecho) sobre la respuesta."}
...
Por ejemplo, la Figura \@ref(fig:rf-vivid-plot), generada con el paquete `r cite_pkg_("vivid", "https://alaninglis.github.io/vivid")`, muestra medidas de la importancia de los predictores (*Vimp*) en la diagonal y de la fuerza de las interacciones (*Vint*) fuera de la diagonal. 
...
```{r rf-vivid-plot, echo=FALSE, fig.cap="Mapa de calor de la importancia e interaciones de los predictores del ajuste mediante bosques aleatorios."}
...
```{r rf-vivid2-plot, echo=FALSE, out.width="80%", fig.cap="Gráfico de red para la importancia e interaciones del ajuste mediante bosques aleatorios."}
...
@breiman2001random sugiere emplear el valor por defecto para `mtry`, la mitad y el doble `r cite_fig(rf-caret-grid)`:

(ref:rf-caret-grid) Evolución de la precisión de un bosque aleatorio con `caret` usando el argumento `tuneGrid`.

```{r rf-caret-grid, fig.cap='(ref:rf-caret-grid)'}
...
Estos métodos son también de los más populares en AE y están implementados en numerosos paquetes de R: `r cite_cran(ada)`, `r cite_cran(adabag)`, `r cite_cran(mboost)`, `r cite_cran(gbm)`, `r cite_pkg_("xgboost", "https://github.com/dmlc/xgboost/tree/master/R-package")`...
...
Para estudiar el efecto de un predictor se pueden generar gráficos de los efectos parciales mediante el método `plot()` `r cite_fig(gbm-plot)`:

(ref:gbm-plot) Efecto parcial del alcohol (panel izquierdo) y la densidad (panel derecho) sobre la respuesta (con `gbm()`).

```{r  gbm-plot, fig.dim = c(12, 6), out.width = "90%", fig.cap='(ref:gbm-plot)'}
...
El método boosting implementado en el paquete `r cite_pkg_("xgboost", "https://github.com/dmlc/xgboost/tree/master/R-package")` es uno de los más populares hoy en día. 

...
```{r margin, echo=FALSE, fig.cap='Ilustración de un clasificador de máximo margen con dos predictores (con datos simulados; los puntos se corresponden con las observaciones y el color, negro o rojo, con la clase). La línea sólida es el hiperplano de máximo margen y los puntos sólidos los vectores de soporte (las líneas discontinuas se corresponden con la máxima distancia del hiperplano a las observaciones).'}
...
```{r margin2, echo=FALSE, fig.cap='(ref:margin2)', fig.dim=c(12, 6), out.width = "90%"}
...
Hay varios paquetes que implementan este procedimiento [e.g. `r cite_cran(e1071)`, @R-e1071; `r cite_cran(svmpath)`, ver @hastie2004entire], aunque se considera que el más completo es `r cite_cran(kernlab)` [@kernlab2004].

La función principal es `r cite_fun(ksvm, kernlab)` y se suelen considerar los siguientes argumentos:

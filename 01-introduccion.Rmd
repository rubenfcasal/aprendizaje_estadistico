...
Podríamos definir la Ciencia de Datos como el conjunto de conocimientos y herramientas utilizados en las distintas etapas del análisis de datos `r cite_fig(esquema)`.
...
También existen paquetes de `R` que permiten utilizar plataformas de ML externas, como por ejemplo `r cite_pkg_("h2o", "https://github.com/h2oai/h2o-3/tree/master/h2o-r ")` o `r cite_cran(RWeka)`.
...
Entre ellos podríamos citar `r cite_pkg_("caret", "https://topepo.github.io/caret")`, `r cite_pkg_("mlr3", "https://mlr3.mlr-org.com")` y `r cite_pkg_("tidymodels", "https://www.tidymodels.org")`.
En la Sección \@ref(caret) se incluye una breve introducción al paquete `r cite_pkg_("caret", "https://topepo.github.io/caret")` [@R-caret; ver también @kuhn2013applied] que será empleado en diversas ocasiones a lo largo del presente libro.
...
Entre ellos estarían R-Commander con el plugin FactoMineR (`Rcmdr`, `RcmdrPlugin.FactoMineR`), `r cite_pkg_("rattle", "https://rattle.togaware.com")` [@R-rattle; ver también @williams2011data] y `r cite_pkg_("radiant", "https://github.com/radiant-rstats/radiant")`.
...
knitr::kable(t(sapply(list(Fit1 = fit1, Fit2 = fit2, Fit3 = fit3), 
    function(x) with(summary(x), 
        c(MSE = mean(residuals^2), R2 = r.squared, R2adj = adj.r.squared)))), "pipe", 
        # col.names = c("Ajuste", "MSE", "R2", "R2adj"),
        caption="Medidas de bondad de ajuste de los modelos polinómicos.", 
...
La recomendación sería por tanto seleccionar los hiperparámetros (el modelo final) tratando de que haya un equilibrio entre el sesgo y la varianza `r cite_fig(biasvar)`.
...
Continuando con el ejemplo anterior, supongamos que queremos emplear regresión polinómica para explicar la valoración de las viviendas a partir del "estatus" de los residentes `r cite_fig(boston-mass)`.
...
Como ya se comentó en la Sección \@ref(metodos-pkgs), el paquete `r cite_pkg_("caret", "https://topepo.github.io/caret")` [*Classification And REgression Training*, @kuhn2008building] proporciona una interfaz unificada que simplifica el proceso de modelado empleando la mayoría de los métodos de AE implementados en R (actualmente admite 239 métodos; ver el [Capítulo 6](https://topepo.github.io/caret/available-models.html) del [manual](https://topepo.github.io/caret) de este paquete).
...
La función principal es `r cite_fun(train, caret)` (descrita en la siguiente subsección), que incluye un parámetro `method` que permite establecer el modelo mediante una cadena de texto.
...
este paquete ha dejado de desarrollarse de forma activa, aunque consideramos que la alternativa `r cite_pkg_("tidymodels", "https://www.tidymodels.org")` [@R-tidymodels] todavía está en fase de desarrollo[^caret-2] y su uso requiere de más tiempo de aprendizaje.
